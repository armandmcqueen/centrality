...
def get_default_configs() -> ControlPlaneSdkConfig:
    return DefaultControlPlaneSdkConfig()
        ...
        with Progress(
            ...
            while True:
                ...
                if sec_count % 1 == 0:
                    ...
                    if len(live_vms) != 0:
                        resp, latest_cpu_measurements = control_plane_sdk.get_latest_cpu_measurements(live_vms)
                        # TODO: Handle errors?

                        for measurement in latest_cpu_measurements:
                            avg_cpu_percent = sum(measurement.cpu_percents) / len(measurement.cpu_percents)
                            progress.update(vm_bars[measurement.vm_id], completed=avg_cpu_percent)
