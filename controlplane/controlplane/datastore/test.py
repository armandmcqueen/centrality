...
def test_metrics(client: DatastoreClient):
    ...
    all_metrics_defined = client.get_cpu_measurements(vm_id=vm_id, start_ts=start_time, end_ts=end_time)
    ...
    first_half_metrics = client.get_cpu_measurements(vm_id=vm_id, start_ts=start_time, end_ts=start_time+datetime.timedelta(seconds=0.5))
    # print(first_half_metrics)
    expected = 5
    matched = len(first_half_metrics) == expected
    output = "PASSED" if matched else "FAILED"
    print(f"[{output}] Expected {expected}, got: {len(first_half_metrics)} (test is potentially flaky)")

    # Get the other half of the measurements
    print()
    print("Testing get_cpu_measurements with start filter")
    second_half_metrics = client.get_cpu_measurements(vm_id=vm_id, start_ts=start_time+datetime.timedelta(seconds=0.5), end_ts=end_time)
    # print(second_half_metrics)
    expected = 5
    matched = len(second_half_metrics) == expected
    output = "PASSED" if matched else "FAILED"
    print(f"[{output}] Expected 5, got: {len(second_half_metrics)} (test is potentially flaky)")
